...
class CustormerJS {
    constructor() {
        ...
        this.getValueOption();
        this.diagAdd = new DialogImport(956, 630, "Thêm mới phiếu nhập kho", '.dialogAdd');
        this.dialogObject = new DialogImport(800, 450, "Chọn đối tượng", '.dialogObject');
        this.dialogProduct = new DialogImport(960, 720, "Chọn hàng hóa", '.dialogProduct');
        this.dialogTickProduct = new DialogImport(960, 720, "Hàng hóa đã chọn", '.dialogTickProduct');
        this.dialogPurpose = new DialogImport(700, 550, "Chọn chứng từ xuất kho điều chuyển", '.dialogPurpose');
        this.confirmCancelAdd = new DialogImport(400, 156, "Dữ liệu chưa được lưu", '.confirmCancelAdd');
        this.tooltipDialog();
    }
    ...
    initEvent() {
        ...
        $(document).on("click", ".check-all-grid", { jsObj: this }, this.checkAll);
    }
    ...
    checkAll(sender) {
        ...
        $(this).css("background-image", "url('/Content/Icons/check.png')");
        $(".check").css("background-image", "url('/Content/Icons/check.png')");
        sender.data["jsObj"].SetDisableButtonToolbar();
    }
